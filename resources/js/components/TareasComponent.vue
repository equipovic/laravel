...
<button class="btn btn-success" type="submit">Guardar cambios "Save"</button>
<button class="btn btn-danger" type="submit" 
@click="cancelarEdicion()">cancelar "cancel"</button>
</form>


<form @submit.prevent="agregar" v-else >
<h3>Agregar tareas "add tasks"</h3>
<input type="text" placeholder="Nombre (Name)" 
class="form-control mb-2" v-model="nota.nombre">
<input type="text" placeholder="Descripcion (Description)"
class="form-control mb-2" v-model="nota.descripcion">
<button class="btn btn-primary" type="submit">Agregar "Add"</button>
</form>

<hr class="mt-3">
<h4>Listado de notas "Note List"</h4>
<ul class="list-group my-2">
    <li class="list-group-item" v-for="(item, index) in notas" :key='index'>
        <span class="badge badge-primary float-lg-right">
          {{item.updated_at}}
        </span>
        <p>{{item.nombre}}</p>
...
//metodo que nos permite visualizar lasnotas existentes que tengasmos 
created(){
   axios.get('/notas')
   .then(res=>{
       this.notas = res.data;
...
methods:{
    agregar(){  //hacemos la validaion para campos no vacios antes de guargadar
              if(this.nota.nombre.trim() === '' || this.nota.descripcion.trim() === ''){
        alert('Debes completar todos los campos antes de guardar');
        return;
      }
       // este codigo nos permite mostrar los datos que seb agregan el el formularios 
      // pero en la consola es para probar si retorna datos 
